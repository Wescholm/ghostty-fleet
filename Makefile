...
.DEFAULT_GOAL := help
.PHONY: help doctor build app run dev quit test lint fmt sync xcode install-cli clean-app
...
dev: app ## Incremental rebuild + relaunch (the inner loop)
	@pkill -f "$(REPO)/$(APP)/Contents/MacOS/ghostty" 2>/dev/null || true
	@sleep 1
	open "$(APP)"
